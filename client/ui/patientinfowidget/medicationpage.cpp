...
void MedicationSearchPage::populateTable(const QJsonArray &arr){
    ...
    if(!kw.isEmpty()){
        ...
        for(const auto &v: arr){ QJsonObject o=v.toObject(); if(o.value("name").toString().compare(kw,Qt::CaseInsensitive)==0 || o.value("generic_name").toString().compare(kw,Qt::CaseInsensitive)==0) exact.push_back(o); }
        if(!exact.isEmpty()) {
            rows = exact;
        } else {
