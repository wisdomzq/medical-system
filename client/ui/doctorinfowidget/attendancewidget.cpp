...
AttendanceWidget::AttendanceWidget(const QString& doctorName, QWidget* parent)
    ...
    l->addWidget(tblAttendance_);
        ...
        connect(btnDoCheckIn_, &QPushButton::clicked, this, [this, leDate, leTime]{
            if (service_) service_->checkIn(doctorName_, leDate->text(), leTime->text());
        });
    ...
    connect(service_, &AttendanceService::checkInResult, this, [this](bool ok, const QString& msg){
        if (ok) { QMessageBox::information(this, tr("提示"), tr("打卡成功")); refreshAttendanceHistory(); }
        else { QMessageBox::warning(this, tr("失败"), msg.isEmpty()? tr("打卡失败"): msg); }
    });
    connect(service_, &AttendanceService::leaveSubmitted, this, [this](bool ok, const QString& msg, const QJsonObject& d){
        showCancelLeave();
        if (ok) {
            int row = tblLeaves_->rowCount(); tblLeaves_->insertRow(row);
    ...
    connect(service_, &AttendanceService::attendanceHistoryReceived, this, [this](const QJsonArray& arr){
        ...
        if (arr.isEmpty() && historyUserTriggered_) {
            ...
            historyUserTriggered_ = false; return;
        }
        ...
        if (historyUserTriggered_) QMessageBox::information(this, tr("提示"), tr("查询历史打卡成功"));
...
AttendanceWidget::AttendanceWidget(const QString& doctorName, CommunicationClient* client, QWidget* parent)
    ...
    btnLeave_ = new QPushButton(tr("请假"), this);
    btnCancel_ = new QPushButton(tr("销假"), this);
    nav->addStretch(); nav->addWidget(btnCheckIn_); nav->addWidget(btnLeave_); nav->addWidget(btnCancel_);
    root->addLayout(nav);
...
void AttendanceWidget::doCheckIn() {
    if (service_) service_->checkIn(doctorName_, QDate::currentDate().toString("yyyy-MM-dd"), QTime::currentTime().toString("HH:mm:ss"));
}

void AttendanceWidget::submitLeave() {
    if (service_) service_->submitLeave(doctorName_, leLeaveDate_? leLeaveDate_->text() : QDate::currentDate().toString("yyyy-MM-dd"), teReason_? teReason_->toPlainText():QString());
}

void AttendanceWidget::refreshActiveLeaves() {
    if (service_) service_->getActiveLeaves(doctorName_);
}

void AttendanceWidget::refreshAttendanceHistory() {
    if (service_) service_->getAttendanceHistory(doctorName_);
}
...
void AttendanceWidget::cancelSelectedLeave() {
    ...
    if (service_) service_->cancelLeave(id);
}
